...
import PatientViewA from './PatientViewA'
...
function PatientAppointments() {
  ...
  const getappointment = async (id) => {
    const res = await fetch(`/patient/appointments/${id}`, {
      method: "GET",
      headers: {
                  ...
                  <tbody>
                    {
                      filter.map((ap) => (
                        <tr key={ap._id}>
                          ...
                          <td>{ap.doctor_schedule.start_time} - {ap.doctor_schedule.end_time}</td>
                          ...
                          <td><div>
                            ...
                            {currentAppointment._id === ap._id ? <PatientViewA data={currentAppointment} onClose={changeCurrentAppointment} updateDashboard={checklogedin}/> : null}
                            ...
                            <button type="button" class="btn btn-danger action-button px-3" onClick={() => updateCancel(ap._id)}><i class="fa-solid fa-xmark"></i></button>
                          </div></td>
                        </tr>
                      ))
                    }
