...
import DoctorCreateSchedule from './DoctorCreateSchedule'
  ...
  useEffect(() => {
    ...
    if (search === '') {
      ...
      let temp = schedules.filter((data) => {
        ...
        return day.includes(search.toLowerCase())
      })
      setFilter([...temp])
    }
  }, [search])
          ...
          <div class="card-body">
            <div class="container">
              <div class="row " >
                ...
                <div class="col-1 ">
                  <button type="button" class="btn btn-success" onClick={() => setAddSchedule(true)}>+</button>
                  {addSchedule ? <DoctorCreateSchedule closeCreate={closeAddButton} updateDashboard={checklogedin} currentSchedule={null} title={'Add Doctor Schedule Data'} btnTitle={'Add'} /> : null}
                </div>
              </div>
                ...
                <table class="table table-hover">
                  <thead>
                    <tr>
                      ...
                      <th scope="col text-center">Consulting Time</th>
                        ...
                        <tr key={schedule._id}>
                          ...
                          <td>{
                            schedule.schedule_status === true ?
                              <button type="button" class="btn btn-primary col-10" onClick={()=>updateStatus(schedule._id,schedule.schedule_status)}> Active </button>
                              : <button type="button" class="btn btn-danger col-10" onClick={()=>updateStatus(schedule._id,schedule.schedule_status)}>InActive</button>
                          }</td>
                          <td><div>
                            <button type="button" class="btn btn-warning action-button" onClick={() => setCurrUpdateSchedule(schedule)}><i class="fa-solid fa-pen-to-square"></i></button>
                            {currUpdateSchedule._id === schedule._id ? <DoctorCreateSchedule closeCreate={closeAddButton} updateDashboard={checklogedin} currentSchedule={currUpdateSchedule} title={'Edit Schhedule'} btnTitle={'Edit'} /> : null}
